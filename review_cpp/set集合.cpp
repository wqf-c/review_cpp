...
//1 集合 有序(默认从小到大) 唯一 (红黑二叉树 这种数据结构的变体) ，不能在指定位置插入元素
void main91()
{
	set<int> set1;
	for (int i = 0; i < 5; i++)
...
int main93()
{
	...
	set1.insert(s1);
	set1.insert(Studentr("张2", 32));
	set1.insert(Studentr("张3", 53));
	set1.insert(Studentr("张4", 34));
...
int main94()
{
	...
	set<int>::iterator it3 = pairIt.first; //获取第一个,找到大于等于5的迭代器
	set<int>::iterator it4 = pairIt.second; //获取第二个，找到大于4的迭代器
